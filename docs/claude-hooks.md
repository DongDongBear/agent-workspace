# Claude Code Hooks：workdpace 实时会话渲染调研
...
因此 workdpace 只消费其 `session_id`、`transcript_path`、`final` 作为失效/活跃提示；不消费 `delta` 作为会话正文。
...
workdpace 仍不把 `last_assistant_message` 写入正式记录；它只把 `Stop` 当作“进行一次最终 JSONL 重读/延迟重试”的信号。用户中断不会触发 `Stop`，API 错误会改触发 `StopFailure`，所以最终校准还应覆盖 `StopFailure`。[StopFailure](https://code.claude.com/docs/en/hooks#stopfailure)
...
结论：不要占用或改写用户的 `statusLine` 来驱动 workdpace。它只能提供回合级信号，且职责是渲染 Claude 自己的状态栏；`MessageDisplay`/`FileChanged` 才是合适的事件面。
...
- 输出通常延迟到下一轮才交给 Claude。workdpace 的 helper 应保持 stdout/stderr 为空，只做本地 IPC。
...
Claude Code
  ...
  └─ Stop/StopFailure ───────────> final-reconcile
                                      │
                                      ▼
workdpace IPC receiver ──50–100ms 合并──> incremental JSONL tailer ──> UI
                                                    ▲
                                                    └── 唯一正文事实源
```
...
4. **早到信号要重试。** `MessageDisplay` 到达但文件无新增完整 record 时，建议在 25/75/200/500ms 做有界重试；`FileChanged` 和 `Stop` 到达时再强制校准。具体间隔是 workdpace 的工程策略，不是 Claude API 保证。
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "/absolute/path/to/workdpace-hook",
            "args": ["session-start"],
            "timeout": 1
          }
        ]
